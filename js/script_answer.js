$(function () {
	...
	function slide() {
		...
		slides
			.find(obj.wbox).css("left", "-100%").stop().animate({ left: "0%" }, 1000, function () {obj.wbox.stop().animate({ left: "100%" }, 1000);
			/*obj라는 객체에 
			* obj.remove()
			* obj.add()
			*라는 메서드를 만들어놓음
			*/ 
				obj.add(slides.eq(current));
				obj.remove(slides.eq(prev)); // 대상이 뒤에 ()들어가야함
				
				//$(".color, .img_logo, .img_text, .box").removeClass("on");->아래처럼 표현됨
				obj.remove(obj.btn_ani)
				obj.remove(obj.text)
				obj.remove(obj.logo)
				obj.remove(obj.cbox)//모든 애니메이트요소 삭제
				
				//$(".slide_btn ul li").eq(current).find(".color").addClass("on");->아래처럼 표현됨
				//obj.add($(".slide_btn ul li").eq(current).find(".color"))->한번더 객체화시키면 아래처럼됨
				obj.add(obj.btn.eq(current).find(obj.btn_ani));
				

				//slides.eq(current).find(".img_logo").addClass("on");-1
				//obj.add(slides.eq(current).find(".img_logo"))-2
				obj.add(slides.eq(current).find(obj.logo))//-3
